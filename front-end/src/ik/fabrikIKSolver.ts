...
const jointDistances: number[] = [];

let rootEndEffectorDistance: number | undefined;

function IKSolver(joints: Object3D[], target: Object3D): Vector3[] {
...
function fabrikIKSolver(jointPositions: Vector3[], targetPosition: Vector3): Vector3[] {
...
function forwardReaching(jointPositions: Vector3[]): void {
  // For i = n - 1, ..., 0  do
...
function backwardReaching(jointPositions: Vector3[]): void {
  // For i = 1, ..., n - 1 do
  for (let idx = 0; idx < jointDistances.length; idx++) {
