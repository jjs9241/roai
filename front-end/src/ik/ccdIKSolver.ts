...
function ccdIKSolver(
  ...
  if(endEffector === null) throw new Error('endEffector is null in ccdIKSolver')
...
function getIKJointRotationAngle(ikJoint: IKJoint): number {
  ...
  const quaternionComponent = ikJoint.quaternion[axisName]
  const axisComponent = axis[axisName as keyof Vector3] as number

  return Math.asin(quaternionComponent / axisComponent) * 2;

}

function clampIKJointRotationAngle(ikJointRotationAngle: number, limit: IKLimit): [number, boolean] {
